    ...
    const result = await query(
      `INSERT INTO sales_invoices
         (invoice_no, custom_invoice_no, client_id, client_name, client_address, vessel_validity,
          shipment_details, pol, pod, container_type, currency, items, total_amount, notes, created_by)
      ...
      [invNo, customInvoiceNo || null, clientId || null, clientName, clientAddress, vesselValidity || null,
       ...
       currency || 'USD', JSON.stringify(items || []), total, notes, req.user.id]
    );
  ...
  try {
    ...
    const result = await query(
      ...
      [clientId || null, clientName, clientAddress, vesselValidity || null,
       ...
       total, notes, status, req.params.id]
    );
...
exports.delete = async (req, res, next) => {
  try {
    const r = await query(
      'UPDATE sales_invoices SET deleted_at = NOW() WHERE id = $1 RETURNING invoice_no',
      [req.params.id]
    );
